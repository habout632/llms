...
# def save_to_huggingface(model, config, repo_id, token):
#     """
...
#
#     hf_model.transformer.wte.weight.data = model.transformer.wte.weight.data
#
#     for i in range(config.n_layer):
#         hf_layer = hf_model.transformer.h[i]
#         custom_layer = model.transformer.h[i]
#
#         hf_layer.attn.c_attn.weight = nn.Parameter(torch.cat([
#             custom_layer.attn.c_q.weight,
#             custom_layer.attn.c_k.weight,
#             custom_layer.attn.c_v.weight
#         ], dim=0))
#         hf_layer.attn.c_proj.weight = custom_layer.attn.c_proj.weight
#
#         hf_layer.mlp.c_fc.weight = custom_layer.mlp.c_fc.weight
#         hf_layer.mlp.c_proj.weight = custom_layer.mlp.c_proj.weight
#
#     hf_model.lm_head.weight = model.lm_head.weight
#
#     api = HfApi()
#     try:
...
#
#     hf_model.save_pretrained(repo_id, push_to_hub=True, token=token)
#
#     tokenizer = AutoTokenizer.from_pretrained("gpt2")
#     tokenizer.save_pretrained(repo_id, push_to_hub=True, token=token)
...
optimizer4 = torch.optim.Adam(scalar_params, lr=0.04, betas=(0.9, 0.95),
                              fused=True)  # note that this learning rate is neither sensitive nor tuned
