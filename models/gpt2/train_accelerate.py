    ...
    for epoch in range(num_epochs):
        ...
        for iteration, (input_batch, target_batch) in enumerate(train_loader):
            try:
                ...
                accelerator.print(f"Epoch {epoch + 1}, Iteration {iteration + 1} ({progress:.2f}%), Train Loss: {loss.item()}, TrainPerplexity: {perplexity.item()}")
                ...
                if (iteration + 1) % 200 == 0:
                    ...
                    with torch.no_grad():
                        ...
                        # accelerator.print(f"Epoch {epoch + 1}, Iteration {iteration + 1} ({progress:.2f}%), Eval Loss: {eval_loss.item()}")

                        eval_loss, eval_perplexity = eval_model(model, val_loader, device)
                        accelerator.print(f"Epoch {epoch + 1}, Iteration {iteration + 1} ({progress:.2f}%), Eval Loss: {eval_loss}, Eval Perplexity: {eval_perplexity}")

                        # Save the model after training
                        # torch.save(model.state_dict(), 'last.pt')
                        # accelerator.print("Last model saved successfully.")

                        # Save the model
                        accelerator.wait_for_everyone()
                        unwrapped_model = accelerator.unwrap_model(model)
                        accelerator.save(unwrapped_model.state_dict(), "last_acc.pt")
                        accelerator.print("Last Acc model saved successfully.")
                        ...
                        if eval_perplexity < best_perplexity:
                            ...
                            # accelerator.print("Best model saved successfully.")

                            accelerator.wait_for_everyone()
                            unwrapped_model = accelerator.unwrap_model(model)
                            accelerator.save(unwrapped_model.state_dict(), "best_acc.pt")
                            accelerator.print("Best Acc model saved successfully.")
                        ...
                        accelerator.print(f"Epoch {epoch + 1}, Iteration {iteration + 1} ({progress:.2f}%), Generated Text: {generated_text}")
                ...
                accelerator.print(f"Epoch {epoch + 1}, Iteration {iteration + 1}, Speed: {iterations_per_second:.2f} iterations per second")
                last_time = current_time
            except Exception as e:
                accelerator.print(f"Error: {e}")
        end_time = time.time()
        epoch_time = end_time - start_time
        accelerator.print(f"Epoch {epoch + 1} execution time: {epoch_time} seconds")
        accelerator.print(f"Epoch {epoch + 1} speed: {len(train_loader) / epoch_time} iterations per second")
...
def main(GPT_CONFIG_124M, OTHER_SETTINGS):
    ...
    # accelerator.print train dataset basic information
    train_dataset_size = len(train_dataloader.dataset)
    num_iterations = len(train_dataloader)
    accelerator.print(f"Train dataset size: {train_dataset_size}")
    accelerator.print(f"Number of iterations per epoch for train: {num_iterations}")

    # accelerator.print val dataset basic information
    val_dataset_size = len(val_dataloader.dataset)
    num_iterations_val = len(val_dataloader)
    accelerator.print(f"Eval dataset size: {val_dataset_size}")
    accelerator.print(f"Number of iterations per epoch for validation: {num_iterations_val}")
    ...
    model, optimizer, data = accelerator.prepare(model, optimizer, train_dataloader, val_dataloader)
